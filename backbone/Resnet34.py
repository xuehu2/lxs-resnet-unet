...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = layers.LayerNormalization(name=bn_name_base + '2a')(x)
    x = layers.Activation('relu')(x)

    x = layers.Conv2D(filters2, kernel_size,padding='same', name=conv_name_base + '2b')(x)

    x = layers.LayerNormalization(name=bn_name_base + '2b')(x)
    x = layers.Activation('relu')(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    shortcut = layers.LayerNormalization(name=bn_name_base + '1')(shortcut)
...
def ResNet34(img):
    ...
    x = layers.LayerNormalization(name='bn_conv1')(x)
    x = layers.Activation('relu')(x)
